...
6.	Multiple lines can be entered into the pgpass.conf file (that you need to add to the correct directory). For example, with a password of “password,” these lines provide access to the database postgres for the user postgres, and access to all databases and users (with this password), respectively: localhost:5432:postgres:postgres:password; *:*:*:*:password. Note that multiple lines can be included in this file.
...
23.	Use the text in Query3.txt to create a table needed for the R. Note this query pulls the correct lat/lon coordinates, and relabels them at ‘lat’ and ‘lon.’
